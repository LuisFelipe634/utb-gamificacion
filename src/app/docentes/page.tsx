...
export default function TeachersPage() {
  ...
  const [activeSection, setActiveSection] = useState<"cursos" | "misiones" | "recompensas" | "historial">("misiones")
      ...
      <div className="grid gap-6 lg:grid-cols-[260px_1fr]">
        ...
        <aside className="h-fit rounded-xl border border-gray-200 bg-white shadow-xs p-4 dark:border-gray-700 dark:bg-gray-800">
          ...
          <div className="mt-3 space-y-1">
            {navItem("cursos", <BookOpen className="h-4 w-4" />, "Cursos asignados", courses.length, "bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300")}
            ...
            {navItem("recompensas", <Gift className="h-4 w-4" />, "Recompensas por revisar", coursePendingRewards.length, "bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300")}
            {navItem("historial", <CheckCircle className="h-4 w-4" />, "Historial", courseMissionHistory.length, "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-300")}
          </div>
          {selectedCourse && (
        ...
        <main className="min-w-0 space-y-6">
          {activeSection === "cursos" && (
            <section className="rounded-xl border border-gray-200 bg-white shadow-xs p-6 dark:border-gray-700 dark:bg-gray-800">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-bold text-gray-900 dark:text-white">Cursos asignados</h2>
                  <p className="text-sm text-gray-500 dark:text-gray-400">Haz clic en un curso para ver sus estudiantes.</p>
                </div>
          ...
          {activeSection === "recompensas" && selectedCourse && (
            <section className="rounded-xl border border-purple-200 bg-purple-50 p-6 dark:border-purple-900 dark:bg-purple-900/20">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-purple-800 dark:text-purple-200">{selectedCourse.code} · {selectedCourse.name}. Revisa las solicitudes de canje de bonificaciones.</p>
                </div>
                <span className="rounded-full bg-white px-3 py-1 text-sm font-semibold text-purple-700">{coursePendingRewards.length}</span>
              </div>
              <div className="mt-4 space-y-3">
                {coursePendingRewards.length > 0 ? coursePendingRewards.map((reward) => (
                ...
                )) : <p className="py-4 text-center text-sm text-purple-600 dark:text-purple-400">No hay solicitudes de recompensa pendientes en este curso.</p>}
              </div>
            </section>
          )}

          {activeSection === "historial" && selectedCourse && (
            <section className="rounded-xl border border-gray-200 bg-white shadow-xs p-6 dark:border-gray-700 dark:bg-gray-800">
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="flex items-center gap-2 text-xl font-bold text-gray-900 dark:text-white"><CheckCircle className="h-5 w-5 text-emerald-500" /> Historial de misiones aceptadas</h2>
                  <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{selectedCourse.code} · {selectedCourse.name}. Misiones verificadas con información del estudiante.</p>
                </div>
                <span className="rounded-full bg-emerald-100 px-3 py-1 text-sm font-semibold text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300">{courseMissionHistory.length}</span>
              </div>
              <div className="mt-4 space-y-3">
                {courseMissionHistory.length > 0 ? courseMissionHistory.map((historyItem) => (
                ...
                )) : <p className="py-4 text-center text-sm text-gray-500 dark:text-gray-400">Aún no hay misiones aceptadas para este curso.</p>}
              </div>
            </section>
          )}
