...
import { AlertTriangle, Award, BookOpen, Flame, Search, Send, Sparkles, Users } from "lucide-react"
...
export default function TeachersPage() {
  ...
  const [notifyFeedback, setNotifyFeedback] = useState<{ id: string; type: "success" | "error"; msg: string } | null>(null)
  ...
  const sendUrgentRoute = async (studentId: string) => {
    ...
    try {
      ...
      setNotifyFeedback({ id: studentId, type: "success", msg: "¡Ruta enviada como notificación urgente!" })
      setTimeout(() => setNotifyFeedback(null), 4000)
    } catch (err) {
      setNotifyFeedback({ id: studentId, type: "error", msg: err instanceof Error ? err.message : "Error al enviar" })
    } finally {
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      {error ? <p className="rounded-lg bg-red-50 p-4 text-red-700">{error}</p> : (
        <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
          <aside className="rounded-xl border border-gray-200 bg-white shadow-xs p-4 dark:border-gray-700 dark:bg-gray-800">
          ...
          {selected ? <section className="space-y-6">
            ...
            <div className="rounded-xl border border-gray-200 bg-white shadow-xs p-6 dark:border-gray-700 dark:bg-gray-800"><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-2xl font-bold text-gray-900 dark:text-white">{selected.name}</h2><p className="text-gray-500">{selected.email} · {selected.program}</p></div><div className="flex items-center gap-3"><span className="rounded-full bg-blue-100 px-3 py-1 text-sm font-semibold text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">Semestre {selected.semester}</span><button type="button" onClick={() => setProfileStudent(selected)} className="rounded-lg bg-blue-600 px-3 py-2 text-sm font-semibold text-white hover:bg-blue-700">Ver perfil completo</button></div></div><div className="mt-6 grid gap-4 sm:grid-cols-4"><div><p className="text-sm text-gray-500">Promedio</p><p className="text-2xl font-bold">{selected.averageGrade.toFixed(1)}</p></div><div><p className="text-sm text-gray-500">Cursos aprobados</p><p className="text-2xl font-bold">{selected.completedCourses}</p></div><div><p className="text-sm text-gray-500">Créditos</p><p className="text-2xl font-bold">{selected.totalCredits} <span className="text-sm font-normal text-gray-500">/ {selected.totalProgramCredits}</span></p></div><div><p className="text-sm text-gray-500">Racha</p><p className="flex items-center gap-1 text-2xl font-bold"><Flame className="h-5 w-5 text-orange-500" />{selected.streak.current} días</p><span className="text-xs text-gray-500">Mejor: {selected.streak.best} días</span></div></div><div className="mt-5"><div className="mb-2 flex justify-between text-sm"><span>Progreso de la carrera</span><strong>{progress}%</strong></div><div className="h-2 rounded-full bg-gray-200 dark:bg-gray-700"><div className="h-2 rounded-full bg-blue-600" style={{ width: `${progress}%` }} /></div></div></div>
            <div className="rounded-xl border border-gray-200 bg-white shadow-xs p-6 dark:border-gray-700 dark:bg-gray-800"><h3 className="text-lg font-bold">Materias seleccionadas este semestre</h3><div className="mt-3 grid gap-2 sm:grid-cols-2">{selected.currentCourses.map((course) => <div key={course.code} className="rounded-lg bg-gray-50 p-3 dark:bg-gray-700/60"><p className="font-semibold">{course.name}</p><p className="text-xs text-gray-500">{course.code} · {course.credits} créditos · {course.period}</p></div>)}{!selected.currentCourses.length && <p className="text-sm text-gray-500">El estudiante aún no ha seleccionado materias.</p>}</div></div>
            <div className="grid gap-6 xl:grid-cols-2"><div className="rounded-xl border border-gray-200 bg-white shadow-xs p-6 dark:border-gray-700 dark:bg-gray-800"><h3 className="flex items-center gap-2 text-lg font-bold"><Sparkles className="h-5 w-5 text-amber-500" /> Ruta recomendada</h3><p className="mt-1 text-sm text-gray-500">Cursos disponibles según los prerrequisitos aprobados.</p><div className="mt-4 space-y-2">{selected.suggestedCourses.map((course) => <div key={course.code} className="flex items-center justify-between rounded-lg bg-gray-50 p-3 dark:bg-gray-700/60"><div><p className="font-semibold">{course.name}</p><p className="text-xs text-gray-500">{course.code} · Semestre {course.semester}</p></div><span className="text-sm text-gray-500">{course.credits} cr.</span></div>)}{!selected.suggestedCourses.length && <p className="text-sm text-gray-500">No hay cursos sugeridos con la información actual.</p>}</div>{selected.suggestedCourses.length > 0 && (<div className="mt-4"><button type="button" onClick={() => sendUrgentRoute(selected.id)} disabled={notifyLoading === selected.id} className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-red-600 px-4 py-2.5 text-sm font-semibold text-white hover:bg-red-700 disabled:opacity-50 shadow-sm"><Send className="h-4 w-4" />{notifyLoading === selected.id ? "Enviando..." : "Enviar como notificación urgente"}</button><p className="mt-2 text-xs text-gray-500 text-center">El estudiante la verá como recuadro flotante en su página principal.</p>{notifyFeedback?.id === selected.id && (<p className={`mt-2 text-center text-sm font-medium ${notifyFeedback.type === "success" ? "text-emerald-600" : "text-red-600"}`}>{notifyFeedback.msg}</p>)}</div>)}</div><div className="rounded-xl border border-gray-200 bg-white shadow-xs p-6 dark:border-gray-700 dark:bg-gray-800"><h3 className="flex items-center gap-2 text-lg font-bold"><Award className="h-5 w-5 text-amber-500" /> Logros e insignias</h3><div className="mt-4 grid gap-2 sm:grid-cols-2">{selected.badges.map((badge) => <div key={badge.name} className="rounded-lg bg-amber-50 p-3 dark:bg-amber-900/20"><p className="font-semibold">{badge.icon} {badge.name}</p><p className="text-xs text-gray-500">{badge.category}</p></div>)}{!selected.badges.length && <p className="text-sm text-gray-500">Aún no registra insignias.</p>}</div></div></div>
