      ...
      {profileStudent && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" role="dialog" aria-modal="true" aria-labelledby="student-profile-title">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl dark:bg-gray-800">
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-4">{[["Semestre", profileStudent.semester], ["Promedio", profileStudent.averageGrade.toFixed(1)], ["Cursos aprobados", profileStudent.completedCourses], ["Insignias", profileStudent.badges.length]].map(([label, value]) => <div key={label} className="rounded-lg bg-gray-50 p-3 dark:bg-gray-700/60"><p className="text-xs text-gray-500">{label}</p><p className="mt-1 text-xl font-bold">{value}</p></div>)}</div>
