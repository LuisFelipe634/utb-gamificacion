...
export default function ProfilePage() {
  ...
  const meritStatus = !data?.profile.meritcoinStudentId
    ? { label: "Sin vincular", cls: "bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300" }
    : !data?.profile.walletAddress
      ? { label: "ID vinculado, sin wallet", cls: "bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300" }
      : { label: "Vinculado a Meritcoin", cls: "bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300" }
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <section className="rounded-xl border border-gray-200 bg-white shadow-xs p-6 dark:border-gray-700 dark:bg-gray-800"><div className="flex items-center justify-between gap-3"><h2 className="text-xl font-bold">Vínculo con Meritcoin</h2><span className={`rounded-full px-3 py-1 text-xs font-semibold ${meritStatus.cls}`}>{meritStatus.label}</span></div><p className="mt-1 text-sm text-gray-500">Usa tu ID de Meritcoin/Moodle (formato STU-3). Si tienes STU-x pero no wallet, Logros provisiona tu wallet custodial automáticamente.</p><div className="mt-4 flex flex-col gap-3"><input value={shownWallet} onChange={(e) => setWalletInput(e.target.value)} placeholder="Wallet 0x..." spellCheck={false} className="flex-1 rounded-lg border border-gray-300 px-3 py-2 font-mono text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white" /><div className="flex flex-col gap-3 sm:flex-row"><input value={shownMeritId} onChange={(e) => setMeritIdInput(e.target.value)} placeholder="ID en Meritcoin/Moodle (ej. STU-3)" spellCheck={false} className="flex-1 rounded-lg border border-gray-300 px-3 py-2 font-mono text-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white" /><button onClick={saveWallet} disabled={walletSaving} className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-50">{walletSaving ? "Guardando..." : "Guardar vínculo"}</button></div></div>{walletMessage && <p className="mt-2 text-sm text-gray-600 dark:text-gray-300">{walletMessage}</p>}</section>
